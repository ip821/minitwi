...
STDMETHODIMP CViewControllerService::OnActivate(IControl* pControl)
{
    ...
    CComPtr<IControl> pSearchControlControl;
    RETURN_IF_FAILED(m_pFormManager->FindForm(CLSID_SearchControl, &pSearchControlControl));
    ATLASSERT(pSearchControlControl);
    if (!m_controlsStack.size())
    {
        CComQIPtr<ISearchControl> pSearchControl = pSearchControlControl;
