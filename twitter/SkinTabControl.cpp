...
STDMETHODIMP CSkinTabControl::GetHWND(HWND *hWnd)
{
	return S_OK;
}

STDMETHODIMP CSkinTabControl::CreateEx(HWND hWndParent, HWND *hWnd)
{
	*hWnd = __super::Create(hWndParent, CRect(), L"", WS_VISIBLE | WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, WS_EX_COMPOSITED);
	return S_OK;
}

STDMETHODIMP CSkinTabControl::PreTranslateMessage(MSG *pMsg, BOOL *pbResult)
{
	return S_OK;
}

STDMETHODIMP CSkinTabControl::UpdateControl(IControl *pControl)
{
	return E_NOTIMPL;
}

STDMETHODIMP CSkinTabControl::EnableCommands(BOOL bEnabled)
{
	return S_OK;
}

STDMETHODIMP CSkinTabControl::AddPage(IControl *pControl)
{
...
void CSkinTabControl::SelectPage(DWORD dwIndex)
{
...
STDMETHODIMP CSkinTabControl::GetCurrentPage(IControl **ppControl)
{
...
STDMETHODIMP CSkinTabControl::RemovePage(IControl *pControl)
{
	return E_NOTIMPL;
}

STDMETHODIMP CSkinTabControl::ActivatePage(IControl *pControl)
{
	auto it = find_if(
		m_pControls.begin(),
		m_pControls.end(),
...
STDMETHODIMP CSkinTabControl::GetPageCount(DWORD *pdwCount)
{
...
STDMETHODIMP CSkinTabControl::GetPage(DWORD dwIndex, IControl **ppControl)
{
	CHECK_E_POINTER(ppControl);
...
STDMETHODIMP CSkinTabControl::OnInitialized(IServiceProvider* pServiceProvider)
{
...
STDMETHODIMP CSkinTabControl::OnShutdown()
{
	for (auto& pControl : m_pControls)
	{
		CComQIPtr<IPluginSupportNotifications> pp = pControl.m_T;
...
STDMETHODIMP CSkinTabControl::Load(ISettings* pSettings)
{
...
STDMETHODIMP CSkinTabControl::Save(ISettings* pSettings)
{
	CHECK_E_POINTER(pSettings);
	for (auto& pControl : m_pControls)
	{
...
LRESULT CSkinTabControl::OnSize(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	CRect clientRect;
